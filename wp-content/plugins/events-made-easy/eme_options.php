...
function eme_options_register() {
   ...
   switch ( $tab ){
	      ...
	      case 'maps' :
	         ...
	         break;
	      case 'other' :
                 $options = array ('eme_thumbnail_size','eme_image_max_width','eme_image_max_height','eme_image_max_size','eme_html_header','eme_html_footer','eme_event_html_headers_format','eme_location_html_headers_format','eme_csv_separator','eme_use_external_url','eme_decimals','eme_localize_price','eme_disable_wpautop');
	         break;
   }

   // allow js only in very specific header settings
   $allow_js_arr=array('eme_html_header','eme_html_footer','eme_payment_form_header_format','eme_payment_form_footer_format','eme_multipayment_form_header_format','eme_multipayment_form_footer_format','eme_payment_succes_format','eme_payment_fail_format');
   foreach ( $options as $opt ) {
	   if (!is_array($opt)) {
...
<table id="rsvp_mail_notify-data" class='form-table'>
   ...
   eme_options_radio_binary ( __ ( 'Verify SMTP certificates?', 'events-made-easy'), 'eme_smtp_verify_cert', __ ( 'Uncheck this option if you have issues sending mail via secure SMTP due to unmatching certificates. Since this in fact defeats the purpose of having certificates, it is not recommended to use it, but sometimes it is needed at sepcific hosting providers.', 'events-made-easy') );
